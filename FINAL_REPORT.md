...
NovaDB should not yet be used as the sole store for irreplaceable data. It now provides tested slotted-page records, checksummed durable B+ tree index images, torn-tail recovery, crash-injection hooks, offline checksummed backup/restore, PBKDF2-backed local identities with role checks, and a row-version MVCC primitive. It does not yet provide SQL-integrated MVCC recovery rules, a complete fsync-boundary crash matrix, external identity and key management/encryption, centralized durable audit retention, durable consensus-backed replication, or a compatibility layer for enterprise SQL dialects. The current optimizer, joins, prepared statements, checksummed page log, LRU buffer pool, prompt governance, compaction, vector index, and Raft-style layer remain prototype/reference implementations.

The project is nevertheless a useful foundation because each limitation is explicit and mapped to a concrete subsystem. The next release should integrate the MVCC primitive into SQL and recovery, complete the crash matrix, and run scheduled backup/restore drills. External identity/key management, durable audit retention, production ANN benchmarking, and real consensus transport should follow only with independent failure evidence. The machine-readable checklist in [`PRODUCTION_GATES.md`](PRODUCTION_GATES.md) keeps those obligations visible.
